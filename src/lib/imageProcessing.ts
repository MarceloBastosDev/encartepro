import { removeBackgroundRMBG } from './removebg-rmbg';
...
// ── Remoção de fundo ──────────────────────────────────────────────────────────
// Usa RMBG-1.4 (Bria AI) — melhor modelo open-source para produtos.
// Sem API, sem limite, roda no browser via ONNX/WASM.

export async function removeBackground(
  file: Blob,
  onProgress?: (text: string) => void,
): Promise<Blob> {
  return removeBackgroundRMBG(file, onProgress);
}
    ...
    try {
      ImageMagick.read(bytes, (image) => {
        ...
        switch (format) {
          ...
          case 'ai':   targetFormat = MagickFormat.Pdf; ext = 'ai'; break;
          default: throw new Error('Unsupported format for ImageMagick');
        }
